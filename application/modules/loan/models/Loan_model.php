...
class Loan_model extends CI_Model {
	...
	{
			    return $this->db->select('count(DISTINCT(ln.loan_id)) as loan_id,count(ln.loan_id) as l_id,ln.*,sum(ln.amount) as amount,sum(ln.repayment_amount) as repayment_amount,p.employee_id,p.first_name,p.last_name')   
            ->from('grand_loan ln')
...
public function report_loan($id,$start_date,$end_date){
...
$this->db->join('grand_loan b', 'a.employee_id = b.employee_id', 'right');
$this->db->join('loan_installment c', 'a.employee_id = c.employee_id', 'right');
