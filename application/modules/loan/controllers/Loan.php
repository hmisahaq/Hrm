	...
	public function create_grandloan(){ 
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->Loan_model->grndloan_create($postData)) { 
				$transaction = [
		     'account_id'               => 4,
		    'transaction_description'   => 'Loan payment Loan ID'.$this->db->insert_id().'Employee ID'.$this->input->post('employee_id',true),
		        'amount'                => $this->input->post('amount',true),
		     'tran_date'                => $this->input->post('date_of_approve',true),
		    'payment_id'                =>$this->input->post('employee_id',true),
		    'create_by_id'              => $this->session->userdata('fullname'),
            ];   
            $this->db->insert('acn_account_transaction', $transaction);
	...
	public function create_installment(){ 
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->Loan_model->installment_create($postData)) { 
				$transaction = [
		    'account_id'                => 5,
		    'transaction_description'   => 'Loan installment Loan ID'.$this->input->post('loan_id',true).'Employee ID'.$this->input->post('employee_id',true),
		    'amount'                    => $this->input->post('payment',true),
		    'tran_date'                 => $this->input->post('date',true),
