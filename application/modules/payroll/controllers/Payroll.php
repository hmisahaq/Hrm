...
class Payroll extends MX_Controller {
	...
	{ 
		...
		$Amount    = number_format($aAmount->gross_salary, 2);
		$startd    = $this->input->post('start_date');
...
// salary with tax calculation
	public function salarywithtax(){
		...
		$this->db->select('*');
		$this->db->from('payroll_tax_setup');
		$this->db->where("$amount BETWEEN `start_amount` AND `end_amount`");
		$query = $this->db->get();
		$rate = $query->row(); 
		$tax = ($rate->rate*$amount)/100;
		$salary = $amount-$tax;
